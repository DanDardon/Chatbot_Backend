...
# 🟢 NUEVO BLOQUE: manejo de frases de mejoría
    ...
    # Generar respuesta ordenada y clara
    respuesta = f"🩺 **Análisis basado en tus síntomas**\n\n"
    respuesta += f"Según los síntomas que mencionas (*{sintomas_str}*), podrías estar presentando:\n\n"
    respuesta += f"### 🧠 *{enfermedad}*\n\n"

    # Descripción
    if "aprendida por retroalimentación" in descripcion_limpia.lower():
        respuesta += f"**📄 Descripción:**\nEnfermedad generada automáticamente por el agente.\n\n"
    else:
        respuesta += f"**📄 Descripción:**\n{descripcion_limpia}\n\n"
    ...
    respuesta += f"**🔎 Nivel de gravedad estimado:**\n**{nivel_gravedad.upper()}** {emoji_alerta} "

    if nivel_gravedad == "alta":
        respuesta += "⚠️ Te recomiendo visitar a un médico cuanto antes.\n\n"
    elif nivel_gravedad == "media":
        respuesta += "🩺 Observa cómo evolucionan tus síntomas. Si empeoran, busca atención médica.\n\n"
    else:
        respuesta += "🙂 Parece ser una condición leve, pero mantente atento a cualquier cambio.\n\n"

    # Consejo de salud
    respuesta += f"**💡 Consejo de salud:**\n{random.choice(consejos_generales)}"
